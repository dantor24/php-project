...
if(empty($Memo) && !preg_match("^[a-zA-Z0-9-_.!?$,/ ]{3,13}$",$Memo)){
    ...
    if(empty($errors)){
        $queryPrepare=$pdo->prepare("UPDATE dossieredu SET Id_Etudiant=:Id_Etudiant, Nom=:Nom, Prenom=:Prenom, Sexe=:Sexe, Adresse=:Adresse, Lieu_de_naissance=:Lieu_de_naissance,
        Date_de_naissance=:Date_de_naissance, Telephone=:Telephone, Email=:Email, Filiere=:Filiere, Niveau=:Niveau,
        Nif_Cin=:Nif_Cin, Personne_de_reference=:Personne_de_reference, Telephone_personne_reference=:Telephone_personne_reference,
        Photo=:Photo, Annee_Academique=:Annee_Academique, Etat=:Etat, Memo=:Memo  WHERE id=:id");
